    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            if (req.method === "GET" && url.pathname === "/status") {
                ...
                json(res, 200, { ok: true });
            } else if (req.method === "POST" && url.pathname === "/generate-summary") {
            ...
            } else if (req.method === "POST" && url.pathname === "/suggest-assignments") {
                const { date, llmIndex = 0 } = await readJson(req);
                if (!date) {
                    json(res, 400, { error: "date required" });
                    return;
                ...
                json(res, 200, { repos });
            } else if (req.method === "GET" && url.pathname === "/last-rebuild-time") {
                const mtime = await engine.getLastRebuildTime();
                json(res, 200, { mtime });
            } else if (req.method === "GET" && url.pathname === "/integration-catalog") {
