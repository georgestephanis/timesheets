...
import { ConfigScreen, ReportScreen, Brand, SidecarProvider, useSidecar } from "@timesheets/ui";

type Screen = "home" | "config" | "reports";

function HomeScreen({ onNavigate }: { onNavigate: (s: Screen) => void }) {
    const { state: sidecarState, error: sidecarError, locateAndStart } = useSidecar();
            ...
            <View style={styles.content}>
                ...
                {statusLine && (
                    <View style={styles.statusRow}>
                        {statusLine.spinner && (
                            <ActivityIndicator size="small" color={Brand.amber} style={styles.spinner} />
                        )}
                        <Text style={[styles.statusText, { color: statusLine.color }]}>{statusLine.text}</Text>
                    </View>
                )}

                <View style={styles.buttons}>
                    <Pressable onPress={() => onNavigate("reports")} style={styles.primaryBtn}>
                        <Text style={styles.primaryBtnText}>Open Reports</Text>
                    </Pressable>
                    <Pressable onPress={() => onNavigate("config")} style={styles.secondaryBtn}>
