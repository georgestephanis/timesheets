...
if ($fromCache) {
    ['events' => $events, 'chrome' => $chrome, 'commits' => $commits, 'external' => $external, 'aiSessions' => $aiSessions] = $cached;
} else {
...
[$fullBucket, $fullUnmatched] = classifyAndAggregate($events, $commits, $external, $aiSessions, $config, $tz, $fullOpts);
...
if (!$fromCache) {
    saveCachedSources($dir, $key, $from, $to, $events, $chrome, $commits, $external, $aiSessions);
}
